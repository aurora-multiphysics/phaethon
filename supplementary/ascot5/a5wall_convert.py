...
def a5wall_convert(filename):
    """Convert the ASCOT5 wall mesh in an HDF5 input file to something MOOSE can parse.

    The output file that is written will have the same basename (stem) as the
    input file but with the ``.inp`` extension, and this file will be written in
    the directory from which the script is run. The output format is Abaqus.
    ...
    mesh.write(str(filename.stem) + '.inp', file_format='abaqus')
